...
const fetchData = async () => {
  ...
  await request.get("admin/wallet-admin").then((res) => {
    dataAdress.value = res.data?.data;
  });
...
// alua
// Adress
const isAdressVisible = ref(false)
const idAdress = ref(null)
const network = ref('')
const adress = ref('')
const isDeleteAdress = ref(false)

const openCreateAdress = (val: any) => {
  isAdressVisible.value = true;
  network.value = '';
  adress.value = '';
  idAdress.value = null;
};
const getEditAdress = (val: any) => {
  isAdressVisible.value = true;
  idAdress.value = val.id;
  network.value = val.network;
  adress.value = val.adress;
};

const createAdress = async () => {
  await request
    .post("admin/wallet-admin/", {
...
const deleteAdress = async () => {
  loadingTable.value = true;
  await request
    .delete(`admin/wallet-admin/${idAdress.value}`)
    .then((res) => {
      notify({
        title: "Thành công",
        text: "Xoá thành công!",
...
const adressUpdate = async () => {
  loadingTable.value = true;
  await request
    .put(`admin/wallet-admin/${idAdress.value}`, {
      network: network.value,
      adress: adress.value,
    })
    .then((res) => {
  ...
  <section>
    <VCard>
      <VTabs v-model="tab">
        ...
        <VTab value="four">
          <span>{{ languagePack.WALLET_DEPOINT }}</span>
        </VTab>
      </VTabs>
          ...
          <VWindowItem value="three">
           ...
           <VWindowItem value="four">
            ...
            <VTable class="text-no-wrap">
              ...
              <!-- 👉 table body -->
              <tbody>
                <tr v-for="result in dataAdress" :key="result.id">
                  <td>
                    <span class="text-base text-high-emphasis" style="text-transform: uppercase;"
                      >{{ result.network }}</span
                    >
                  </td>
                  <td>
                  ...
                  <td>
                    <VBtn color="warning" @click="getEditAdress(result)">
                      <VIcon
                        icon="bx-lock-open-alt"
                        style="margin-right: 5px"
                      />
    ...
    <!-- alua -->
    <!-- adress MODAL -->
    <VDialog v-model="isAdressVisible" max-width="600">
      <!-- Dialog Activator -->
      <!-- Dialog Content -->
      <VCard :title="languagePack.INTEREST_TITLE">
        <DialogCloseBtn
        ...
        <VCardText class="d-flex justify-end gap-2">
          ...
          <VBtn @click="idAdress ? adressUpdate() : createAdress()">
            Save
          </VBtn>

          <VBtn v-if="idAdress" color="error" @click="isDeleteAdress = true">
            Delete
          </VBtn>
        </VCardText>
      </VCard>
...
<!--  -->
    ...
    <VDialog v-model="isDeleteAdress" persistent class="v-dialog-sm">
      <!-- Dialog Activator -->
      ...
      <VCard title="Delete">
        ...
        <VCardText class="d-flex justify-end gap-2">
          ...
          <VBtn @click="deleteAdress"> Delete </VBtn>
        </VCardText>
      </VCard>
    </VDialog>
  </section>
